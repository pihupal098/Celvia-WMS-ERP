...
def analyze_reviews_with_ai(review_data_text):
    ...
    """
    try:
        # YAHAN CHANGE KIYA HAI - Sabse stable model lagaya hai
        model = genai.GenerativeModel('gemini-pro')
        response = model.generate_content(prompt)
        clean_json = response.text.replace("```json", "").replace("```", "").strip()
        return json.loads(clean_json)
    except Exception as e:
        return {"error": str(e), "raw_response": "API Failure or Output Parsing Error"}
        ...
        if review_col:
            ...
            if st.button("🚀 Run Deep Analysis (Seller Data)", type="primary"):
                with st.spinner("Calculating 100-Point Health Score & Issue Matrix... 🧠"):
                    ...
                    if "error" in ai_data:
                        st.error(f"❌ AI Parsing Error: {ai_data['error']}")
                    else:
...
with tab2:
    ...
    <div style="background: rgba(56, 189, 248, 0.1); border: 1px dashed #38bdf8; padding: 15px; border-radius: 10px;">
        <strong>Phase 3 Network Hook:</strong> Jab aap 'Scan Competitor' dabayenge, toh backend ek <code>GET</code> request bhejeha <em>RapidAPI Scraper</em> ko. Scraper 200 recent reviews dega, jisko Gemini AI 100-Point Algorithm se process karega.
    </div>
    """, unsafe_allow_html=True)
    ...
    if st.button("🔍 Scan Competitor Weaknesses"):
        if not product_link:
            ...
            with st.spinner("Connecting to Scraper API... Bypassing Security..."):
                time.sleep(2) 
